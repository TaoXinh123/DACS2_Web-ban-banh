...
<?php
    ...
    $uname = $phone = $address = $email = $password= "";
    $passwordCriteria = "";
    $fnameCriteria = "";
    $lnameCriteria = "";
    $unameCriteria = "";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["phone"])) {
            ...
            $fname = test_input($_POST["phone"]);
            // check if name only contains letters and whitespace
            if (!preg_match("/^[0-9]{10}$/",$fname)) {
                $fnameCriteria = "Wrong format";
            }
            else
            {
                $fnameOK = true;
        ...
        if (empty($_POST["address"])) {
            ...
            $lname = test_input($_POST["address"]);
            // check if name only contains letters and whitespace
        ...
        {   
            if($unameOK == true && $passwordOK == true && $fnameOK == true && $lnameOK == true && $emailOK == true)
            {
                ...
                $sql = "INSERT INTO user (uname, pass, address, phone, email, vkey,verified,isAdmin)
                VALUES ('$uname', '$passHash', '$fname', '$lname', '$email', '$vkey', '0','0')";
                echo $sql;
...
<html lang="en-MU">
    <head>
        <meta charset="utf-8">
        <title>La Boulangerie | Join Us</title>
